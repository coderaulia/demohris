...
const SUPABASE_ACTIONS = new Set<string>([
    // Future domain actions can be moved here as backend cutovers happen.
    'auth/session',
]);
...
function resolveSource(action: string, target: BackendTarget): TransportSource {
    ...
    if (target === 'supabase') return SUPABASE_ACTIONS.has(action) ? 'supabase' : 'legacy';
...
// Placeholder for Supabase domain cutovers. For now, it falls back to legacy contracts.
async function requestSupabase<TSchema>(
    ...
    return requestLegacy(action, payload, method, schema, accessToken);
}
