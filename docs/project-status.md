...
| Backend Endpoint Cutover (Phase C) | In progress | Migrate low-risk endpoint groups to Supabase-backed reads first | Modules read slice + LMS read slice (`enrollments list/get/my-courses`, `progress/get`) completed and parity-hardened; first TNA read-only slice (`tna/summary`) is now cut over with source switch; TNA mutations/reporting and LMS mutations remain legacy | High | Team | Run TNA smoke with Supabase admin credentials, keep TNA route off, then migrate next TNA read reporting slice (`gaps-report` or `lms-report`) |
