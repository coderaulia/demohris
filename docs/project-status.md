...
| KPI & Assessment Module (React Shell) | In progress (reporting + add-record workflows) | Legacy-parity management reporting workflow for KPI and assessment summaries | Summary tabs and grouped department/team breakdown are available; HR/superadmin can now add KPI records and HR/manager can add assessment needs from employee detail, but live smoke coverage for those writes is still pending | High | Team | Run `qa:kpi:cutover` with seeded KPI admin creds and add focused workflow smoke for KPI record and TNA need creation |
